...
async function getPersonalProjectsFromNotion(): Promise<PersonalProject[]> {
  try {
    ...
    if (!token) {
      console.error("No PERSONAL_TOKEN found")
      return []
    }

    if (!databaseId) {
      console.error("No PERSONAL_DATABASE_ID found")
      return []
    }
  ...
  try {
    ...
    return NextResponse.json({
      ...
      metadata: {
        ...
        errors: projects.length === 0 ? ["No projects found or database connection failed"] : [],
        warnings: [],
      },
    })
  } catch (error) {
